...
import type { FieldDateTimePickerProps } from './datetimePicker';
import { FieldDatetimePicker } from './datetimePicker';
import styles from './index.module.scss';

type DateTimeRangeValue = [Date | null, Date | null];

export interface FieldDateTimeRangePickerProps
    extends Omit<
        FieldBaseProps,
    ...
    onChange: (dates?: DateTimeRangeValue) => void;
    disabled?: [boolean, boolean];
    maxDate?: Date;
    dateTimePickerProps?: {
        start?: Omit<FieldDateTimePickerProps, 'value' | 'onChange'>;
        end?: Omit<FieldDateTimePickerProps, 'value' | 'onChange'>;
    };
...
const Day = (props: PickersDayProps<Dayjs> & { selectedDates?: [Date | Dayjs | null, Date | Dayjs | null] }) => {
    ...
    return (
        <CustomPickersDay
            ...
            isLastDay={isLastDay}
            isSingle={selectedDates.some((date) => !(isDayjs(date) && !isNaN(date.get('milliseconds'))))}
        />
    );
};
//eslint-disable-next-line
export const FieldDateTimeRangePicker = forwardRef<HTMLInputElement, FieldDateTimeRangePickerProps>((props, ref) => {
    const {
        label,
        fullWidth,
        helperText,
